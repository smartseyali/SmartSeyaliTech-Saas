...
(function () {
    ...
    window.StorefrontInstance = {
        ...
        async init() {
            ...
            // Dynamically detect tenant slug from URL path: /stores/{slug}/
            const pathParts = window.location.pathname.split('/').filter(Boolean);
            const storesIndex = pathParts.indexOf('stores');
            let slug = 'organic'; // fallback
            if (storesIndex !== -1 && pathParts[storesIndex + 1]) {
                slug = pathParts[storesIndex + 1];
            }
        ...
        applyBranding(data) {
            document.title = `${data.name || 'Storefront'} | Premium Organic`;
            ...
            if (data.primary_color) {
                // Use the primary CSS variable for consistent theming
        ...
        addToCart(product, qty = 1) {
            ...
            this.saveCart();
            this.pushNotification(`${product.name} archived to cart.`);
        },
    ...
    // Expose a helper for layout to refresh cart counts after injection
    window.updateCartCounts = () => window.StorefrontInstance.syncCartUI();
})();
