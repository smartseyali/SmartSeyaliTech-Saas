...
const COMPANY = "SmartSeyali Tech";
const SUPPORT_EMAIL = "support@smartseyali.com";
const WEBSITE = "https://www.smartseyali.com";
...
function SectionBlock({
  id, number, icon: Icon, title, children,
}: {
  ...
  return (
    <motion.div id={id} {...fadeUp()} className="bg-white border border-gray-100 rounded-2xl p-8 scroll-mt-24">
      <div className="flex items-start gap-4 mb-5">
        ...
        <div>
          <span className="text-xs font-semibold text-primary-600 uppercase tracking-widest block mb-0.5">{number}</span>
          <h2 className="text-xl font-bold text-gray-900">{title}</h2>
        </div>
      </div>
      <div className="pl-14 space-y-3 text-gray-600 text-sm leading-relaxed">{children}</div>
    </motion.div>
  );
}
      ...
      <section className="relative bg-white pt-32 pb-16 lg:pt-40 lg:pb-20 border-b border-gray-100">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} className="max-w-3xl">
            <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-blue-50 border border-blue-100 text-primary-600 text-xs font-semibold uppercase tracking-wide mb-6">
              <span className="w-1.5 h-1.5 rounded-full bg-primary-600 animate-pulse" />
              Legal
            </div>
            <h1 className="text-4xl sm:text-5xl lg:text-6xl font-bold text-gray-900 leading-[1.1] tracking-tight mb-6">
              Terms &<br /><span className="text-primary-600">Conditions</span>
            </h1>
            <p className="text-lg text-gray-500 leading-relaxed max-w-2xl mb-6">
              Please read these terms carefully before using the SmartSeyali business platform. By accessing or using our
            ...
            <div className="flex flex-wrap items-center gap-4 text-sm text-gray-400">
              ...
              <span className="flex items-center gap-1.5"><Globe className="w-3.5 h-3.5" />Applies to: {WEBSITE}</span>
              <span className="hidden sm:block w-1 h-1 rounded-full bg-gray-300" />
              <span className="flex items-center gap-1.5"><Scale className="w-3.5 h-3.5" />Governed by Indian Law</span>
            </div>
          </motion.div>
        </div>
      </section>
        ...
        <div className="flex gap-12 items-start">
          ...
          >
            <div className="bg-white border border-gray-100 rounded-2xl p-6">
              ...
              <nav className="space-y-1">
                {SECTIONS.map((s, i) => (
                  <a key={s.id} href={`#${s.id}`}
                    className="flex items-center gap-2 py-1.5 px-2 rounded-lg text-sm text-gray-500 hover:text-primary-600 hover:bg-blue-50 transition-colors group"
                  >
                    <span className="text-[10px] font-bold text-gray-300 group-hover:text-primary-600 w-4 shrink-0">
              ...
              <div className="mt-6 pt-5 border-t border-gray-100">
                <Link href="/policy" className="flex items-center gap-2 text-xs text-gray-500 hover:text-primary-600 transition-colors group">
          ...
          <div className="flex-1 min-w-0 space-y-6">
            <SectionBlock id="acceptance" number="01" icon={CheckCircle} title="Acceptance of Terms">
              <p>
                By accessing or using {COMPANY}&apos;s SmartSeyali platform (&quot;Service&quot;), you (&quot;User&quot;, &quot;Customer&quot;, or
            ...
            <SectionBlock id="description" number="02" icon={Globe} title="Platform Description">
              <p>SmartSeyali is a multi-tenant Software-as-a-Service (SaaS) business management platform that provides modular applications including, but not limited to:</p>
              <div className="mt-3 grid sm:grid-cols-2 gap-3">
                {["E-Commerce & Storefront", "Point of Sale (POS)", "CRM & Customer Management", "Sales Management",
                  "Inventory & Warehouse", "Purchase Management", "HRMS & Payroll", "Finance & Accounting",
                  "WhatsApp Business Integration", "Website Builder"].map((item) => (
                  <div key={item} className="flex items-center gap-2 text-gray-700">
                    <CheckCircle className="w-4 h-4 text-primary-600 shrink-0" /><span>{item}</span>
                  </div>
                ))}
              </div>
              <p className="mt-4">Features and modules are subject to change. {COMPANY} reserves the right to modify, discontinue, or add features with reasonable notice to subscribers.</p>
            </SectionBlock>

            <SectionBlock id="accounts" number="03" icon={Users} title="Account Registration & Security">
              <SubSection title="3.1 Registration">
                <Ul items={["You must provide accurate, complete, and current information when registering",
              ...
              <SubSection title="3.2 Account Security">
                <Ul items={["You are responsible for maintaining the confidentiality of your login credentials",
              ...
              <SubSection title="3.3 Super Admin Access">
                <p>Company owners and super admins have elevated permissions. They are solely responsible for managing user roles, permissions, and data access within their tenant workspace.</p>
              </SubSection>
            </SectionBlock>

            <SectionBlock id="billing" number="04" icon={CreditCard} title="Subscription & Billing">
              <SubSection title="4.1 Subscription Model">
                <p>SmartSeyali operates on a per-module subscription model. Each installed module is billed separately at the listed monthly price. Core modules included with all accounts are free of charge.</p>
              </SubSection>
              <SubSection title="4.2 Billing Cycle & Payments">
                <Ul items={["Subscriptions are billed monthly on the first day of each billing cycle",
              ...
              <SubSection title="4.3 Free Trials">
                <p>Certain modules may offer a free trial period (as indicated on the module listing). At the end of the trial, the module will require an active subscription to continue use. No payment information is required to start a trial.</p>
              </SubSection>
              <SubSection title="4.4 Refund Policy">
                <p>Monthly subscriptions are non-refundable once a billing cycle has begun. If you cancel during a cycle, access continues until the end of that period. We may issue credits at our discretion for service interruptions exceeding 24 hours caused by our infrastructure.</p>
              </SubSection>
              <SubSection title="4.5 Price Changes">
                <p>We reserve the right to change subscription prices. We will provide at least 30 days&apos; advance notice via email and platform notification before any price change takes effect.</p>
              </SubSection>
            </SectionBlock>
                ...
                <div className="space-y-2">
                  {["Use the Service for any unlawful, fraudulent, or deceptive purpose",
                    ...
                    <div key={item} className="flex items-start gap-2.5 text-gray-600">
                      <XCircle className="w-4 h-4 text-red-400 shrink-0 mt-0.5" /><span>{item}</span>
                    </div>
                  ))}
                </div>
              </SubSection>
              <p className="mt-2">Violation of this policy may result in immediate suspension or termination of your account without refund.</p>
            </SectionBlock>

            <SectionBlock id="ip" number="06" icon={Lock} title="Intellectual Property">
              <SubSection title="6.1 Our Intellectual Property">
                <p>The SmartSeyali platform, including all software, source code, design, trademarks, logos, and documentation, is the exclusive intellectual property of {COMPANY}. You receive a limited, non-exclusive, non-transferable licence to use the Service during your subscription.</p>
              </SubSection>
              <SubSection title="6.2 Your Data">
                <p>You retain full ownership of all data, content, and business information you upload or create within the platform (&quot;Customer Data&quot;). You grant {COMPANY} a limited licence to process and store Customer Data solely to provide the Service.</p>
              </SubSection>
              <SubSection title="6.3 Feedback">
                <p>Any feedback, suggestions, or ideas you submit may be used by {COMPANY} without obligation or compensation to you.</p>
              </SubSection>
            </SectionBlock>
            ...
            <SectionBlock id="data" number="07" icon={Shield} title="Data & Privacy">
              <p>
                ...
                <Link href="/policy" className="text-primary-600 underline">Privacy Policy</Link>, which is incorporated into these Terms by reference. Key commitments:
              </p>
              <Ul items={["Your data is stored with strict tenant isolation — no other customer can access your data",
            ...
            <SectionBlock id="availability" number="08" icon={RefreshCw} title="Service Availability & SLA">
              <p>We target 99.5% monthly uptime for the SmartSeyali platform, excluding scheduled maintenance windows (announced at least 24 hours in advance).</p>
              <Ul items={["Scheduled maintenance is typically performed on Sundays between 02:00–06:00 IST",
            ...
            <SectionBlock id="liability" number="09" icon={AlertTriangle} title="Limitation of Liability">
              ...
              <Ul items={["The Service is provided 'as is' without warranties of any kind, express or implied",
              ...
              <p className="mt-3">Some jurisdictions do not allow limitation of implied warranties or liability; in such cases, our liability is limited to the greatest extent permitted by applicable law.</p>
            </SectionBlock>

            <SectionBlock id="indemnification" number="10" icon={Shield} title="Indemnification">
              <p>You agree to indemnify, defend, and hold harmless {COMPANY}, its directors, officers, employees, and agents from any claims, damages, losses, liabilities, and expenses (including legal fees) arising from:</p>
              <Ul items={["Your use or misuse of the Service", "Your violation of these Terms",
            ...
            <SectionBlock id="termination" number="11" icon={XCircle} title="Termination">
              <SubSection title="11.1 Termination by You">
                <p>You may cancel your subscription at any time from the Billing section of your dashboard. Cancellation takes effect at the end of the current billing period. You retain access until that date.</p>
              </SubSection>
              <SubSection title="11.2 Termination by Us">
                <p>We may suspend or terminate your account immediately if:</p>
                <Ul items={["You materially breach these Terms and fail to cure within 7 days of notice",
              ...
              <SubSection title="11.3 Effect of Termination">
                <p>Upon termination, your access to the Service ceases. You may request an export of your Customer Data within 14 days of termination. After 30 days, your data will be permanently deleted.</p>
              </SubSection>
            </SectionBlock>

            <SectionBlock id="governing-law" number="12" icon={Scale} title="Governing Law & Dispute Resolution">
              <p>These Terms are governed by and construed in accordance with the laws of India, specifically:</p>
              <Ul items={["Information Technology Act, 2000 and its amendments",
                "Digital Personal Data Protection Act, 2023 (DPDPA)", "Indian Contract Act, 1872"]} />
              <p className="mt-3">Any disputes arising out of or in connection with these Terms shall be subject to the exclusive jurisdiction of the courts in <strong>Tiruppur, Tamil Nadu, India</strong>.</p>
              <p className="mt-3">Before initiating legal proceedings, both parties agree to attempt good-faith resolution through direct negotiation for a period of 30 days.</p>
            </SectionBlock>

            <SectionBlock id="changes" number="13" icon={RefreshCw} title="Changes to Terms">
              <p>When material changes are made:</p>
              <Ul items={["The 'Effective Date' at the top of this page will be updated",
              ...
              <p className="mt-3">If you do not agree to the revised Terms, you must cease using the Service and may cancel your subscription before the effective date for a pro-rated refund of any unused prepaid subscription.</p>
            </SectionBlock>
                ...
                {[
                  ...
                  <div key={label} className="flex items-start gap-3 p-4 bg-gray-50 rounded-xl border border-gray-100">
                    ...
                    <div>
                      ...
                      {href ? <a href={href} className="text-sm text-gray-700 hover:text-primary-600 transition-colors">{value}</a>
            ...
            <motion.div {...fadeUp()} className="flex flex-col sm:flex-row items-center justify-between gap-4 pt-4">
              <p className="text-xs text-gray-400">Last updated: {EFFECTIVE_DATE} &nbsp;·&nbsp; {COMPANY}</p>
              <Link href="/policy" className="inline-flex items-center gap-2 text-sm font-semibold text-primary-600 hover:underline">
