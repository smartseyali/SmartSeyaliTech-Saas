            ...
            <div className="h-full px-3 flex items-center justify-between gap-4">
                ...
                <div className="flex-1 max-w-md relative group hidden sm:block">
                    ...
                    <div className="absolute right-3 top-1/2 -translate-y-1/2 flex items-center gap-1 opacity-30 select-none">
                        <span className="text-xs font-bold border border-slate-400 px-1 rounded">/</span>
                    </div>
                </div>
                ...
                <div className="flex items-center gap-4">
                    ...
                    <Popover>
                        ...
                        <PopoverContent align="end" sideOffset={8} className="w-[340px] p-4 rounded-2xl shadow-2xl border border-slate-100">
                            <div className="flex items-center justify-between mb-4">
                                <p className="text-[13px] font-extrabold tracking-widest text-slate-500 uppercase">{PLATFORM_CONFIG.name} Apps</p>
                                <Link to="/apps" className="text-[13px] font-extrabold tracking-widest text-blue-600 hover:text-blue-700 flex items-center gap-1 uppercase">
                                    All Apps <ExternalLink className="w-3.5 h-3.5" />
                                </Link>
                            </div>
                            <div className="grid grid-cols-4 gap-2">
                                    ...
                                    .map(mod => {
                                        ...
                                        return (
                                            ...
                                            >
                                                ...
                                                <span className={`text-[13px] font-bold text-center leading-tight uppercase tracking-tight ${isSubscribed ? 'text-slate-600' : 'text-slate-500'}`}>
                                                    {mod.name}
                                                </span>
                                            </Link>
                                        );
                                ...
                                <Link to="/apps" className="flex flex-col items-center gap-1.5 p-2 rounded-xl hover:bg-slate-50 transition-all group">
                                    ...
                                    <span className="text-[13px] font-bold text-slate-500 text-center">All Apps</span>
                                </Link>
                            </div>
                        </PopoverContent>
                    </Popover>
                    ...
                    <DropdownMenu>
                        <DropdownMenuTrigger asChild>
                            <Button variant="ghost" className="h-9 px-2 rounded-lg hover:bg-slate-100 flex items-center gap-3 transition-all">
                                ...
                                <div className="flex flex-col items-start justify-center hidden md:flex text-left">
                                    <span className="font-bold text-slate-900 text-[15px] leading-none mb-1">
                                        {userEmail.split('@')[0]}
                                    </span>
                                    <span className="font-bold text-[13px] text-slate-500 leading-none uppercase tracking-wider">
                                        {userRole}
                                    </span>
                                </div>
                        ...
                        <DropdownMenuContent align="end" className="w-56 rounded-xl p-2 shadow-2xl border-slate-200">
                            <DropdownMenuLabel className="px-2 py-2 text-xs font-bold  tracking-wider text-slate-500">Account</DropdownMenuLabel>
