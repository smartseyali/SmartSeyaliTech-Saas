...
export default function VerifyEmail() {
    ...
    const verifyToken = async (t: string) => {
        setStatus("loading");
        try {
            const { data, error } = await supabase.rpc("customer_verify_email", { p_token: t });
            if (error) throw new Error(error.message);

            if (data?.already_verified) {
                setStatus("success");
                setMessage("Your email was already verified.");
                setCustomerName(data.email);
            } else if (data?.verified) {
                setStatus("success");
                setMessage("Your email has been verified successfully!");
                setCustomerName(data.full_name || data.email);
            }
        } catch (err: any) {
            setStatus("error");
            ...
            <div className="w-full max-w-md text-center space-y-8">
                ...
                {status === "success" && (
                    <div className="bg-white rounded-3xl border border-emerald-200 p-10 shadow-sm space-y-6">
                        ...
                        <div>
                            ...
                            {customerName && <p className="text-sm text-slate-500 mt-2">Welcome, {customerName}</p>}
                            <p className="text-sm text-emerald-600 font-medium mt-1">{message}</p>
                        </div>
                        <p className="text-sm text-slate-500">You can now place orders and enjoy the full shopping experience.</p>
                        <div className="flex flex-col gap-3">
                            <Button className="w-full h-12 rounded-xl bg-blue-600 hover:bg-blue-700 text-white font-bold shadow-lg shadow-blue-600/20 gap-2"
                                onClick={() => navigate("/store/login")}>
                                <ShoppingBag className="w-4 h-4" /> Continue to Store
                            </Button>
                ...
                {status === "error" && (
                    <div className="bg-white rounded-3xl border border-rose-200 p-10 shadow-sm space-y-6">
                        ...
                        <Button className="w-full h-12 rounded-xl bg-blue-600 hover:bg-blue-700 text-white font-bold shadow-lg shadow-blue-600/20"
                            onClick={() => navigate("/store/login")}>
                            Go to Login
                        </Button>
                    </div>
