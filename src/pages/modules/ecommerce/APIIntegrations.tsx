            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-8 pb-8 border-b border-slate-100">
                <div className="space-y-4">
                    <div className="flex items-center gap-3">
                        ...
                        <span className="text-xs font-bold uppercase tracking-widest text-slate-400">Ecosystem & Bridges</span>
                    </div>
                ...
                {connectors.map((c, i) => (
                    <div key={i} className="group bg-white p-8 rounded-[32px] border border-slate-200 shadow-sm hover:shadow-2xl hover:border-blue-200 transition-all duration-300">
                        <div className="flex justify-between items-start mb-8">
                            ...
                            <div className="px-4 py-1.5 bg-emerald-50 text-emerald-600 rounded-full text-[9px] font-bold uppercase tracking-widest border border-emerald-100 shadow-sm">
                                {c.status}
                            </div>
                        </div>
                        <div className="space-y-6">
                            <div className="space-y-1">
                                <p className="text-[10px] font-bold text-blue-600 uppercase tracking-widest">{c.type}</p>
                                <h3 className="text-2xl font-bold text-slate-900 leading-none tracking-tight">{c.name}</h3>
                            </div>
                            <p className="text-sm font-medium text-slate-500 leading-relaxed italic">"{c.desc}"</p>

                            <div className="pt-8 flex justify-between items-center border-t border-slate-50">
                                <span className="text-[9px] font-bold text-slate-400 uppercase tracking-[0.2em] flex items-center gap-2">
                                    <RefreshCw className="w-3.5 h-3.5 animate-spin-slow opacity-40 text-blue-600" /> Latency: {c.delay}
                                </span>
                                <Button variant="ghost" className="h-10 px-5 text-[10px] font-bold uppercase tracking-widest text-slate-400 hover:text-blue-600 hover:bg-blue-50 rounded-xl transition-all gap-2">
