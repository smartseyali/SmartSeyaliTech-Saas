    ...
    return (
        <div className="p-8 space-y-8 animate-in fade-in duration-500 pb-20">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-8 pb-10 border-b border-slate-100 relative">
                <div className="space-y-4">
                    <div className="flex items-center gap-3">
                        ...
                        <span className="text-xs font-black uppercase tracking-[0.3em] text-slate-500 italic">E-commerce Orders</span>
                    </div>
                    <h1 className="text-4xl font-black tracking-tight text-slate-950 uppercase italic leading-none">{t("Orders")} Overview</h1>
                    <p className="text-sm font-bold text-slate-500 uppercase tracking-widest">{filtered.length} total orders found</p>
                </div>
                <div className="flex items-center gap-4">
                    <Button variant="outline" className="h-14 px-8 rounded-2xl bg-white border-slate-200 text-slate-600 font-bold hover:bg-slate-50 transition-all gap-3 shadow-sm" onClick={load}>
                        <RefreshCw className={cn("w-5 h-5", loading && "animate-spin")} /> Refresh Data
                    </Button>
                    ...
                    <button key={s.key} onClick={() => setActiveStatus(s.key)}
                        className={cn(
                            ...
                            activeStatus === s.key
                                ...
                                : "text-slate-500 bg-white hover:text-slate-900 border-slate-100 hover:border-slate-200"
                        )}>
                        {s.label}
                        <span className={cn(
                            "px-3 py-1 rounded-lg text-[9px] font-mono",
                            activeStatus === s.key ? "bg-white/20 text-white" : "bg-slate-100 text-slate-600"
                        )}>
            ...
            <div className="relative group/search">
                <Search className="absolute left-8 top-1/2 -translate-y-1/2 w-6 h-6 text-slate-400 group-focus-within/search:text-blue-600 transition-colors" />
                <input type="text" value={search} onChange={e => setSearch(e.target.value)}
                    placeholder={`Search by ${t("Order").toLowerCase()} #, ${t("Customer").toLowerCase()} name, or phone...`}
                    className="w-full h-16 pl-20 pr-8 rounded-[24px] border border-slate-100 bg-white text-sm font-bold uppercase tracking-tight focus:outline-none focus:ring-8 focus:ring-blue-500/5 focus:border-blue-500 transition-all shadow-sm placeholder:text-slate-300" />
            </div>
            ...
            <div className="bg-white rounded-[40px] border border-slate-100 shadow-sm overflow-hidden hover:shadow-2xl transition-all duration-500">
                {loading ? (
                    <div className="flex flex-col items-center justify-center py-40 gap-6">
                        ...
                        <p className="text-[10px] font-bold uppercase tracking-[0.4em] text-blue-600/60">Loading Orders...</p>
                    </div>
                ) : filtered.length === 0 ? (
                    <div className="text-center py-40 bg-slate-50/20">
                        <div className="w-28 h-28 rounded-[40px] bg-white mx-auto mb-8 flex items-center justify-center text-slate-200 shadow-sm border border-slate-100">
                            <Package className="w-14 h-14" />
                        </div>
                        <p className="text-xl font-black text-slate-900 uppercase tracking-tight">No Orders Found</p>
                        <p className="text-[10px] font-bold text-slate-500 uppercase tracking-[0.3em] mt-2 italic">Try adjusting your filters or search terms.</p>
                    </div>
                ) : (
                    <div className="overflow-x-auto">
                        <table className="w-full">
                            <thead>
                                <tr className="bg-slate-50/50 text-[10px] font-black uppercase tracking-[0.3em] text-slate-500 border-b border-slate-100">
                                    <th className="px-10 py-6 text-left">Order #</th>
                                    <th className="px-10 py-6 text-left">Customer</th>
                                    <th className="px-10 py-6 text-left">Total Amount</th>
                                    <th className="px-10 py-6 text-left">Items</th>
                                    <th className="px-10 py-6 text-left">Status</th>
                                    <th className="px-10 py-6 text-left">Order Date</th>
                            ...
                            <tbody className="divide-y divide-slate-50">
                                {filtered.map(o => {
                                    ...
                                    return (
                                        <tr key={o.id} className="hover:bg-slate-50/50 transition-all group cursor-pointer" onClick={() => navigate(`/apps/ecommerce/orders/${o.id}`)}>
                                            ...
                                            <td className="px-10 py-5">
                                                <div className="space-y-1">
                                                    ...
                                                    <p className="text-[9px] font-bold text-slate-500 uppercase tracking-[0.2em] italic">{o.customer_phone}</p>
                                                </div>
                                            </td>
                                            <td className="px-10 py-5">
                                                <div className="space-y-1">
                                            ...
                                            <td className="px-10 py-5">
                                                <span className="text-[11px] font-bold text-slate-500 italic">Order Items</span>
                                            </td>
                                            <td className="px-10 py-5">
                                            ...
                                            <td className="px-10 py-5">
                                                <div className="space-y-1">
                                                    ...
                                                    <p className="text-[9px] font-bold text-slate-400 uppercase tracking-widest opacity-60">Order Date</p>
                                                </div>
                                            </td>
                                            <td className="px-10 py-5" onClick={(e) => e.stopPropagation()}>
                                                <div className="flex items-center gap-3 justify-end">
                                                    <Link to={`/apps/ecommerce/orders/${o.id}`}
                                                    ...
                                                    {nextStatus && (
                                                        <button onClick={() => advanceStatus(o)} disabled={updatingId === o.id}
                                                            className="h-12 px-5 rounded-[16px] bg-blue-600 text-white text-[10px] font-black uppercase tracking-[0.2em] shadow-xl shadow-blue-600/20 transition-all hover:bg-black active:scale-95 disabled:opacity-50">
