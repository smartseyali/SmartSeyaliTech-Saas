...
export default function OrderDetail() {
    ...
    if (loading) return (
        <div className="bg-[#f8fafc] min-h-screen flex flex-col items-center justify-center gap-6 pt-24 font-sans text-slate-900">
            ...
            <p className="text-xs font-bold  tracking-widest text-slate-500 ">Syncing Records...</p>
        </div>
    );
    ...
    if (!order) return (
        <div className="bg-[#f8fafc] min-h-screen flex flex-col items-center justify-center p-6 space-y-8 pt-24 font-sans">
            ...
            <div className="text-center space-y-4">
                <h1 className="text-4xl font-bold text-slate-900  tracking-tighter ">Order Not Found</h1>
                <p className="text-slate-500 font-medium ">This record does not exist in our registry.</p>
            </div>
            <Button
                onClick={() => navigate(storeLink("/orders"))}
                className="rounded-2xl bg-slate-900 text-white px-10 h-14 font-bold  tracking-widest text-[13px] shadow-2xl transition-all  border-none"
            >
                Back to Registry
            </Button>
        </div>
        ...
        <div className="bg-[#f8fafc] min-h-screen pb-40 font-sans pt-24 text-slate-900">
            <div className="max-w-7xl mx-auto px-6 py-20">
                ...
                <div className="mb-20 space-y-12">
                    <button
                        onClick={() => navigate(storeLink("/orders"))}
                        className="flex items-center gap-3 text-xs font-bold  tracking-widest text-slate-500 hover:text-blue-600 transition-all "
                    >
                    ...
                    <div className="flex flex-col md:flex-row md:items-end justify-between gap-12 pb-16 border-b border-slate-100">
                        <div className="space-y-6">
                            <div className="flex items-center gap-3">
                                ...
                                <span className="text-xs font-bold  tracking-widest text-slate-500 ">Order Identified</span>
                            </div>
                            <h1 className="text-6xl md:text-8xl font-bold text-slate-900 tracking-tighter  leading-[0.8] ">
                                Order <span className="text-blue-600">Details</span>
                            </h1>
                            <p className="text-slate-500 font-bold text-[13px] tracking-[0.5em]   opacity-50">
                                REFERENCE: #{order.order_number || order.id?.slice(0, 10).toUpperCase()}
                            </p>
                        </div>
                        <div className="flex items-center gap-6">
                            <div className={cn("px-10 py-5 rounded-2xl text-xs font-bold  tracking-widest border shadow-2xl ", statusColor)}>
                                {order.status}
                            </div>
                ...
                <div className="grid grid-cols-1 xl:grid-cols-12 gap-12">
                    ...
                    <div className="xl:col-span-8 space-y-12">
                        ...
                        >
                            <h3 className="text-[13px] font-bold  tracking-widest text-slate-900 border-b border-slate-100 pb-8 mb-16 ">Order Progress</h3>
                            <div className="flex items-start">
                                {STATUS_STEPS.map((step, idx) => (
                                    <div key={step.id} className="flex-1 flex flex-col items-center text-center gap-6 relative">
                                        ...
                                        <p className={cn("text-xs font-bold  tracking-widest ", idx <= displayStepIndex ? "text-slate-900" : "text-slate-200")}>
                                            {step.label}
                                        </p>
                                    </div>
                                ))}
                        ...
                        >
                            <h3 className="text-[13px] font-bold  tracking-widest text-slate-900 border-b border-slate-100 pb-8 ">
                                    ...
                                    <div key={item.id} className="flex gap-10 items-center group">
                                        ...
                                        <div className="flex-grow space-y-3">
                                            <div className="flex items-center gap-3">
                                                ...
                                                <span className="text-[13px] font-bold text-slate-300  tracking-widest  leading-none">Verified Inventory</span>
                                            </div>
                                            <h4 className="font-bold text-2xl text-slate-900  tracking-tighter  group-hover:text-blue-600 transition-colors">{item.name}</h4>
                                            <div className="flex items-center gap-6 pt-1">
                                                <span className="text-xs font-bold text-slate-500  tracking-widest ">{item.variant_name || "Standard Unit"}</span>
                                                <span className="text-xs font-bold text-blue-600  tracking-widest ">{item.quantity} Unit(s) @ ₹{Number(item.price_at_time || item.price).toLocaleString()}</span>
                                            </div>
                                        </div>
                                        <p className="font-bold text-3xl text-slate-900 shrink-0 tracking-tighter  tabular-nums">₹{Number(item.total_price || (item.quantity * (item.price_at_time || item.price))).toLocaleString()}</p>
                                    </div>
                                ))}
                            </div>

                            <div className="pt-12 border-t border-slate-100 space-y-6">
                                <div className="flex justify-between items-center text-xs font-bold  tracking-widest text-slate-500  font-bold">
                                    <span>Subtotal</span>
                                ...
                                <div className="flex justify-between items-end pt-12 border-t border-slate-100">
                                    <div className="space-y-4">
                                        <p className="text-xs font-bold  tracking-[0.5em] text-blue-600  leading-none">Total Settlement</p>
                                        <p className="text-5xl md:text-7xl font-bold text-slate-900 tracking-tighter leading-none  tabular-nums">₹{Number(order.grand_total || order.total_amount).toLocaleString()}</p>
                                    </div>
                                    <div className="flex items-center gap-4 px-10 py-5 bg-slate-900 rounded-3xl border-none shadow-2xl">
                                        <ShieldCheck className="w-5 h-5 text-blue-400" />
                                        <span className="text-xs font-bold  tracking-widest text-white ">Protected</span>
                                    </div>
                                </div>
                            </div>
                        </motion.div>
                    ...
                    <div className="xl:col-span-4 space-y-12">
                        ...
                        >
                            ...
                            <div className="space-y-12 relative z-10">
                                <div className="flex items-center gap-4">
                                    ...
                                    <h4 className="text-[13px] font-bold  tracking-widest text-white/30 border-b border-white/5 pb-6 w-full ">Delivery Sector</h4>
                                </div>
                                <div className="space-y-8">
                                    ...
                                    <div className="flex items-center gap-4 p-6 bg-white/5 rounded-[2rem] border border-white/10 text-xs font-bold  tracking-widest  text-blue-400">
                        ...
                        >
                            <div className="flex items-center gap-4">
                                ...
                                <h4 className="text-[13px] font-bold  tracking-widest text-slate-300 ">Settlement</h4>
                            </div>
                            <div className="space-y-8">
                                <div className="flex justify-between items-center pb-4 border-b border-slate-50">
                                    <span className="text-xs font-bold  tracking-widest text-slate-500 ">Method</span>
                                    <span className="text-[13px] font-bold text-slate-900  tracking-widest ">
                                        {order.payment_method === "cod" ? "Manual Transfer (COD)" : (order.payment_method || "Online Matrix")}
                                    </span>
                                </div>
                                <div className="flex justify-between items-center pb-4 border-b border-slate-50">
                                    <span className="text-xs font-bold  tracking-widest text-slate-500 ">Status</span>
                                    <span className={cn("text-xs font-bold  px-6 py-2.5 rounded-xl border  tracking-widest", order.payment_status === "paid" ? "bg-slate-900 text-blue-400 border-slate-800" : "bg-slate-50 text-slate-500 border-slate-100")}>
                                        {order.payment_status || "Processing"}
                                    </span>
                                </div>
                                <div className="flex justify-between items-center">
                                    <span className="text-xs font-bold  tracking-widest text-slate-500 ">Date</span>
                                    <span className="text-[13px] font-bold text-slate-900  tracking-widest ">
                        ...
                        >
                            ...
                            <div className="text-center space-y-8 relative z-10">
                                ...
                                <div className="space-y-4">
                                    <h5 className="font-bold  text-3xl leading-none  tracking-tighter">Support <br />Center</h5>
                                    <p className="text-xs text-white/50 font-bold  tracking-widest ">Available 24/7 for assistance</p>
                                </div>
                                <Button
                                    onClick={() => window.open(`https://wa.me/${settings?.whatsapp_number || "919000000000"}`, '_blank')}
                                    className="w-full bg-white text-blue-600 hover:bg-slate-900 hover:text-white rounded-3xl h-20 font-bold  tracking-widest text-[13px] border-none  transition-all shadow-2xl"
                                >
                                    Uplink Now
                                </Button>
                            </div>
