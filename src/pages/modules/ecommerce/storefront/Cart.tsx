            ...
            <div className="bg-[#f8fafc] min-h-screen flex items-center justify-center p-6 font-sans">
                <div className="bg-white rounded-[4rem] text-center p-20 max-w-xl w-full space-y-10 shadow-2xl shadow-slate-200/50 border border-slate-50">
                    ...
                    <div className="space-y-4">
                        ...
                        <p className="text-slate-500 text-sm font-medium ">"Operational grid awaiting asset initialization. No active deployments identified."</p>
                    </div>
                    <Button
                ...
                <div className="flex flex-col md:flex-row md:items-end justify-between gap-8 mb-20 border-b border-slate-100 pb-12">
                    <div className="space-y-4">
                        <div className="flex items-center gap-3">
                            ...
                            <span className="text-xs font-bold  tracking-widest text-slate-500">Inventory Lock</span>
                        </div>
                    ...
                    <p className="text-xs font-bold text-slate-500  tracking-widest bg-white px-8 py-4 rounded-2xl border border-slate-50 shadow-xl shadow-slate-200/30 ">
                    ...
                    <div className="lg:col-span-8 space-y-8">
                                        ...
                                        >
                                            ...
                                            <div className="flex-grow space-y-6 text-center sm:text-left">
                                                <div className="space-y-3">
                                                    ...
                                                    {item.variant_name && <span className="inline-block px-4 py-1.5 bg-blue-50 text-blue-600 text-[13px] font-bold  tracking-widest rounded-lg border border-blue-100">{item.variant_name}</span>}
                                                </div>
                                                ...
                                                <div className="flex items-center justify-center sm:justify-start gap-8">
                                                    <div className="flex items-center bg-slate-50 rounded-2xl border border-slate-100 p-1.5 shadow-inner">
                                                        <button onClick={() => updateQuantity(item.product_id, item.variant_id, -1)} className="w-12 h-12 flex items-center justify-center text-slate-500 hover:text-slate-900 hover:bg-white rounded-xl transition-all font-bold text-xl">−</button>
                                                        <span className="w-12 text-center font-bold text-xl tracking-tighter tabular-nums text-slate-900">{item.quantity}</span>
                                                        <button onClick={() => updateQuantity(item.product_id, item.variant_id, 1)} className="w-12 h-12 flex items-center justify-center text-slate-500 hover:text-slate-900 hover:bg-white rounded-xl transition-all font-bold text-xl">+</button>
                                                    </div>
                                                    <div className="h-6 w-px bg-slate-100" />
                                                    <button onClick={() => removeFromCart(item.product_id, item.variant_id)} className="text-xs font-bold  tracking-widest text-slate-300 hover:text-blue-600 transition-all flex items-center gap-3 ">
                                            ...
                                            <div className="text-right sm:w-40 space-y-2">
                                                ...
                                                <p className="text-xs font-bold text-slate-300  tracking-widest text-center sm:text-right ">₹{item.price.toLocaleString()} / UNIT</p>
                                            </div>
                                        </motion.div>
                                    );
                                })}
                        ...
                        <div className="bg-slate-900 rounded-[3rem] p-10 flex items-center gap-8 border border-white/5 shadow-2xl">
                            ...
                            <p className="text-xs font-bold text-white/70  tracking-widest leading-relaxed  pr-4">
                                ...
                                <br /><span className="text-xs text-white/30">Protocol: RAPID-TRANSIT-V2 ACTIVE</span>
                            </p>
                        </div>
                    </div>
                    ...
                    <div className="lg:col-span-4 space-y-8 lg:sticky lg:top-36">
                        <div className="bg-white rounded-[4rem] p-12 shadow-2xl shadow-slate-200/50 border border-slate-50 space-y-10">
                            ...
                            <div className="space-y-6">
                                <div className="flex justify-between text-[13px] font-bold text-slate-500  tracking-widest ">
                        ...
                        <div className="flex items-center gap-6 px-10 py-8 bg-slate-50 rounded-[2.5rem] border border-slate-100 shadow-sm">
                            ...
                            <div className="space-y-1">
                                <p className="text-[13px] font-bold  tracking-widest text-slate-900 ">Encrypted Secure Bridge</p>
                                <p className="text-[13px] font-bold text-slate-300  tracking-widest flex items-center gap-2">
