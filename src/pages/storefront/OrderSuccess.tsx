...
export default function OrderSuccess() {
    ...
    if (loading) return (
        <div className="bg-[#fafaf9] min-h-screen flex flex-col items-center justify-center gap-6">
            ...
            <p className="text-[10px] font-black uppercase tracking-[0.3em] text-[#14532d]/40">Securing Harvest Details...</p>
        </div>
    );
        ...
        <div className="bg-[#fafaf9] min-h-screen pt-28 pb-40 font-sans">
            ...
            <div className="max-w-4xl mx-auto px-6">
                ...
                >
                    ...
                    <div className="space-y-4">
                        <div className="flex items-center justify-center gap-2">
                            ...
                            <span className="text-[10px] font-black uppercase tracking-[0.4em] text-[#14532d]/40">Acquisition Successful</span>
                        </div>
                        <h1 className="text-4xl md:text-6xl font-black text-[#14532d] tracking-tighter uppercase leading-none">Harvest <br /><span className="text-[#f97316]">Confirmed</span></h1>
                        <p className="text-slate-400 font-medium text-lg italic mt-4 max-w-lg mx-auto leading-relaxed">Your organic selection has been reserved and is now entering the fulfillment protocol.</p>
                        ...
                        {order && (
                            <div className="inline-flex items-center gap-3 bg-[#f8fafc] px-8 py-3 rounded-2xl border border-slate-100 text-[10px] font-black uppercase tracking-widest text-[#14532d]/60 shadow-sm mt-4">
                                Harvest Reference: <span className="text-[#14532d] select-all">#{order.order_number || id?.slice(0, 8).toUpperCase()}</span>
                            </div>
                        )}
                    </div>
                </motion.div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mt-12">
                    ...
                    >
                        <h3 className="text-xs font-black uppercase tracking-[0.3em] text-[#14532d] border-b border-slate-50 pb-6">Protocol Timeline</h3>
                        <div className="space-y-0">
                            {[
                                { icon: CheckCircle2, title: "Harvest Placed", sub: "Order verified & reserved" },
                                { icon: Package, title: "Nurturing Phase", sub: "Preparing items for dispatch" },
                                { icon: Truck, title: "Vault Dispatched", sub: "Leaving regional fulfillment center" },
                                { icon: Truck, title: "Transit Protocol", sub: "En-route to your coordinates" },
                                { icon: Home, title: "Organic Arrival", sub: "Produce delivered successfully" },
                            ].map((step, idx) => {
                    ...
                    <div className="space-y-8">
                        ...
                        >
                            ...
                            <h3 className="text-xs font-black uppercase tracking-[0.3em] text-white/40 border-b border-white/5 pb-6 mb-8 flex items-center gap-2">
                                <Truck className="w-4 h-4 text-[#f97316]" /> Destination Details
                            ...
                            <div className="space-y-6">
                                <div className="space-y-1">
                                    <p className="text-[10px] font-black uppercase tracking-widest text-white/30 leading-none">Vault Courier</p>
                                    <p className="text-sm font-bold opacity-80">{order?.courier_name || "Logistics Assignment Pending"}</p>
                                </div>
                                <div className="space-y-1">
                                    <p className="text-[10px] font-black uppercase tracking-widest text-white/30 leading-none">Registered Coordinates</p>
                                    <p className="text-base font-bold leading-relaxed">{typeof order?.shipping_address === 'string' ? order.shipping_address : (order?.shipping_address?.line1 || "Coordinates pending verification")}</p>
                                </div>
                                <div className="flex justify-between items-end pt-6 border-t border-white/5">
                                    <div className="space-y-1">
                                        <p className="text-[10px] font-black uppercase tracking-widest text-[#f97316]">Settlement Amount</p>
                        ...
                        >
                            ...
                            >
                                Track Current Harvest <ArrowRight className="w-4 h-4 ml-2" />
                            </Button>
                            <Button
                ...
                >
                    <div className="flex items-center gap-3 text-[10px] font-black uppercase tracking-widest text-[#14532d]/30">
                        <ShieldCheck className="w-4 h-4 text-[#f97316]" /> Encrypted Session
                    </div>
                    <div className="flex items-center gap-3 text-[10px] font-black uppercase tracking-widest text-[#14532d]/30">
                        <MessageCircle className="w-4 h-4 text-[#f97316]" /> Concierge Direct
                    </div>
                    <div className="flex items-center gap-3 text-[10px] font-black uppercase tracking-widest text-[#14532d]/30">
                        <Download className="w-4 h-4 text-[#f97316]" /> Digital Certificate
                    </div>
                </motion.div>
            </div>
        </div>
