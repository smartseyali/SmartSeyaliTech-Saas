    ...
    injectHeader() {
        ...
        if (!document.getElementById('nav-dropdown-style')) {
            ...
            style.textContent = `
                ...
                .nav-cat-trigger     { padding:12px 16px; font-size:13px; font-weight:700; color:var(--text-muted); cursor:pointer; display:flex; align-items:center; gap:5px; border-bottom:2px solid transparent; transition:all .2s; user-select:none; }
                .nav-cat-trigger:hover,
                .nav-cat-wrap:hover .nav-cat-trigger { color:var(--primary); border-bottom-color:var(--primary); }
                .nav-cat-trigger svg { transition:transform .2s; height: 10px; width: 10px; }
                .nav-cat-wrap:hover .nav-cat-trigger svg { transform:rotate(180deg); color: var(--primary); }
                
                /* THE DROPDOWN */
                .nav-dropdown        { display:none; position:absolute; top:100%; left:0; min-width:260px; background:white; border-radius:12px; box-shadow:0 12px 36px rgba(0,0,0,.15); z-index:1000; padding:10px 0; border:1px solid var(--border); margin-top: 4px; border-top: 3px solid var(--primary); }
                
                /* Bridge to prevent hover loss */
                .nav-dropdown::before { content:''; position:absolute; top:-12px; left:0; right:0; height:12px; }
                
                .nav-cat-wrap:hover .nav-dropdown { display:block; animation:navFadeIn .2s ease; }
                @keyframes navFadeIn { from{opacity:0;transform:translateY(8px)} to{opacity:1;transform:translateY(0)} }
                
                .nav-dropdown a      { display:flex; align-items:center; gap:12px; padding:10px 20px; font-size:13px; font-weight:600; color:var(--text); transition:all .15s; white-space:nowrap; }
                .nav-dropdown a:hover { background:rgba(40,116,240,.08); color:var(--primary); transform: translateX(4px); }
                .nav-dropdown a img   { width:32px; height:32px; border-radius:50%; object-fit:cover; border:1px solid var(--border); background: #f8f8f8; }
                .nav-dropdown a .cat-dot { width:32px; height:32px; border-radius:50%; background:rgba(40,116,240,.1); display:flex; align-items:center; justify-content:center; font-size:14px; flex-shrink:0; }
                .nav-dropdown .view-all { border-top:1px solid var(--bg); margin-top:6px; color:var(--primary) !important; font-weight:800 !important; }
            `;
